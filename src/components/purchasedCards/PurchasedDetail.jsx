...
export const PurchasedDetail = ({ id, image, name, scale, position, anyadirCartaBaraja }) => {
	console.log("🚀 ~ file: PurchasedDetail.jsx:27 ~ PurchasedDetail ~ scale:", scale);

	const { cartasCompradas } = useContext(contextBaraja);

	const [scaleFocus, setScaleFocus] = useState(scale);
	...
	return (
		<>
			<Container
				...
				pointerover={() => setScaleFocus({ x: 0.16, y: 0.16 })}
				pointerout={() => setScaleFocus({ x: 0.14, y: 0.14 })}
				onclick={(e) => (e.detail === 2 ? anyadirCartaBaraja(e.target.id) : null)}
				cursor="pointer"
				scale={scaleFocus}
				// anchor={anchor}
			>
				{imgSprt &&
					imgSprt.map(({ id, texture }, index) => {
						...
						return <Sprite name={name} texture={texture} ref={id === 0 ? referenciaSprite : null} key={index} scale={scale} />;
					})}
